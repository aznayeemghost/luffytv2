...
export default function MovieWatchPage({ movieId }: { movieId: number }) {
  ...
  return (
    <div className="fade-in">
      ...
      {/* Player controls bar */}
      <div className="bg-[#111111] rounded-none lg:rounded-xl p-4 mt-1 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
          ...
          <div className="flex gap-4 overflow-x-auto scroll-container pb-2">
            {topCast.map(person => (
              <div key={person.id} className="shrink-0 text-center w-[90px]">
                <div className="w-[80px] h-[80px] rounded-full bg-[#1a1a1a] overflow-hidden mx-auto mb-2 border-2 border-white/[0.06]">
                  {person.profile_path ? (
