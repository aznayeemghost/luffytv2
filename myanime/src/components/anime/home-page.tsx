...
export default function HomePage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      try {
        ...
        if (cached) {
          ...
          if (Date.now() - parsed.ts < CACHE_TTL) {
            const d = parsed.data;
            if (d.trendingAll) setTrendingAll(d.trendingAll);
            if (d.anilistTrending) setAnilistTrending(d.anilistTrending);
            if (d.anilistPopular) setAnilistPopular(d.anilistPopular);
            if (d.anilistTopRated) setAnilistTopRated(d.anilistTopRated);
            if (d.popularMovies) setPopularMovies(d.popularMovies);
            if (d.popularTV) setPopularTV(d.popularTV);
            if (d.topRatedMovies) setTopRatedMovies(d.topRatedMovies);
            if (d.topRatedTV) setTopRatedTV(d.topRatedTV);
            if (d.trendingMovies) setTrendingMovies(d.trendingMovies);
            if (d.animeData) setAnimeData(d.animeData);
            setLoading(false);
            return;
          }
        }
      ...
      // Cache for 5 min so navigation back is instant
      try {
