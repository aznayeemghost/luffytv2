...
export default function LiveWatchPage(props: LiveWatchProps) {
          ...
          if (currentRoute.page === "live-watch" && currentRoute.matchId === mid) {
            useAppStore.setState({
              route: {
                ...
                matchDamitvEmbed: match.damitvEmbed || (currentRoute as any).matchDamitvEmbed,
  ...
  // Fetch stream URLs — simplified: direct embeds from props + resolver API
  useEffect(() => {
    if (!props.matchId) return;
    const fetchStreams = async () => {
      setLoadingStreams(true);
      try {
        // ── BUILD DIRECT EMBED STREAMS FROM PROPS ──
        const directStreams: StreamInfo[] = [];

        // 1. DamiTV embed — direct from props (the API provides the ready-to-use embed URL)
        const damitvEmbedUrl = props.matchDamitvEmbed || "";
        const damitvId = props.matchDamitvId || "";
        if (damitvEmbedUrl) {
          directStreams.push({
            id: `damitv-direct-${damitvId || "embed"}`,
            streamNo: directStreams.length + 1,
            language: "English",
            hd: true,
            m3u8Url: "",
            quality: "720p",
          ...
          // Construct embed URL from damitvId
          directStreams.push({
            id: `damitv-direct-${damitvId}`,
            streamNo: directStreams.length + 1,
            language: "English",
            hd: true,
            m3u8Url: "",
            ...
            viewers: 0,
            provider: "damitv",
            embedUrl: `https://dami-tv.pro/embed/?id=${encodeURIComponent(damitvId)}`,
            corsEnabled: false,
            referer: "https://dami-tv.pro/",
            streamType: "embed" as const,
          });
        ...
        // 2. WatchFooty streams — direct embed URLs from props
        const wfDirectStreams: StreamInfo[] = wfStreamsFromProps
          .filter((s: any) => s.url && !s.isRedirect)
          .map((s: any, i: number) => ({
            id: `wf-direct-${s.id || i}`,
            streamNo: directStreams.length + i + 1,
        ...
        // ── FETCH FROM RESOLVER API (StreamedPK PRIMARY, then streamfree, etc.) ──
        ...
        if (damitvId) params.set("damitvId", damitvId);
        if (damitvEmbedUrl) params.set("damitvEmbed", damitvEmbedUrl);
        ...
        // ── MERGE ALL STREAMS ──
        // Order: StreamedPK (from resolver) FIRST, then DamiTV direct, then WatchFooty direct
        const allStreams = [...resolverStreams, ...directStreams, ...wfDirectStreams];
        ...
        // Sort: StreamedPK (provider "streamed") FIRST, then DamiTV, then other embeds, then M3U8
        const providerOrder: Record<string, number> = { streamed: 1, damitv: 2, watchfooty: 3, streamfree: 4, sportsembed: 5 };
        ...
        if (uniqueStreams.length > 0) {
          ...
          // Prefer StreamedPK streams first, then DamiTV, then any embed, then M3U8
          const streamedStream = uniqueStreams.find((s: StreamInfo) => s.provider === "streamed" && s.embedUrl);
          const damitvStream = uniqueStreams.find((s: StreamInfo) => s.provider === "damitv" && s.embedUrl);
          ...
          const best = streamedStream || damitvStream || embedStream || corsStream || m3u8Stream || uniqueStreams[0];
  ...
  useEffect(() => {
    ...
    if (activeStream?.streamType === "embed" && activeStream?.embedUrl) {
      ...
      // Let the embed load naturally — no timeout. The onLoad/onError handlers
      // on the iframe will handle success/failure cases.
      return;
    }
