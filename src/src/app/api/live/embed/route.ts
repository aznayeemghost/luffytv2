...
// Simplified: just use the embed URL directly. No HLS/PPV fetching.
async function resolveDamiTV(matchId: string, prebuiltEmbed?: string): Promise<StreamResult[]> {
  const results: StreamResult[] = [];
  try {
    // Use prebuilt embed URL if available (from API's s.embed field), otherwise construct from matchId
    const embedUrl = prebuiltEmbed || `https://dami-tv.pro/embed/?id=${encodeURIComponent(matchId)}`;
    results.push({
...
export async function GET(req: Request) {
  ...
  const damitvEmbed = url.searchParams.get("damitvEmbed") || "";
  ...
  // ALSO try searching by team names as fallback (even if we have sources, might find more)
  if ((homeTeam || awayTeam) && parsedSources.length === 0) {
  ...
  // ── PRIORITY 3: DamiTV ──
  // Use damitvEmbed directly if available (the correct embed URL from the API),
  // otherwise fall back to constructing from damitvId
  if (damitvEmbed) {
    // Direct embed URL from API — just use it, no need to resolve anything
    resolvePromises.push(resolveDamiTV(damitvId || "direct", damitvEmbed));
  } else if (damitvId) {
    resolvePromises.push(resolveDamiTV(damitvId));
  }
  // Also try channelCode as a DamiTV source if no damitvId
  if (!damitvId && !damitvEmbed && channelCode) {
    resolvePromises.push(resolveDamiTV(channelCode));
  }
